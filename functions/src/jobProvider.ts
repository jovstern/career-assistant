...
const roleMatches = (jobTitle: string, roles: string[]) => {
...
const locationMatches = (job: MockTemplate, locations: string[]) => {
  if (locations.length === 0) return true
  const loc = job.location.toLowerCase()
  return locations.some((l) => {
    const wanted = l.toLowerCase()
    if (wanted === 'remote') return job.remote === 'remote'
...
export class MockJobProvider implements JobProvider {
  async search(criteria: JobSearchCriteria): Promise<JobPosting[]> {
    return TEMPLATES.filter((t) => {
      if (!roleMatches(t.title, criteria.roles)) return false
      if (!locationMatches(t, criteria.locations)) return false
      if (criteria.remote !== 'any' && t.remote !== criteria.remote) return false
      if (criteria.minSalary && t.salaryMin < criteria.minSalary) return false
      return true
    }).map((t) => {
